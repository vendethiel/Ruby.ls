{spawn, exec} = require \child_process
require! <[LiveScript fs]>

# ANSI Terminal Colors.
bold  = '\33[0;1m'
red   = '\33[0;31m'
green = '\33[0;32m'
reset = '\33[0m'

tint = (text, color ? green) -> color + text + reset

slobber = (path, code) ->
  spit path, code
  say '* ' + tint path, bold

minify = ->
  {parser, uglify} = require \uglify-js
  ast = parser.parse it
  ast = uglify.ast_mangle  ast
  ast = uglify.ast_squeeze ast
  uglify.gen_code ast

build = ({min = false} = {}) ->
  js = [slurp 'node_modules/prelude-ls/prelude.js']

  for file in fs.readdir-sync 'src/'
    try
      ls = "\n#{slurp "src/#file"}\n"
      compiled = LiveScript.compile ls, {+bare}
      slobber "lib/#{file - '.ls'}.js" compiled
      js.push compiled
    catch {message}
      say "Error compiling #{tint file, bold} : " + tint message, red
      return

  js .= join '\n'

  slobber "lib/Sugar.js" """
    // Sugar.LS 0.1.0
    // Copyright (c) 2012 Nami-Doc
    // Released under the MIT License
    // raw.github.com/gkz/prelude-ls/master/LICNSE
    #{ if min then minify js else js}
  """

task \build 'build lib/sugar.js from src/' ->
  build!

task \test 'run test/' -> runTests!

function runTests
  global.LiveScript = LiveScript
  startTime = Date.now!
  passedTests = failedTests = 0

  for name, func of require \assert then let
    global[name] = -> func ...; ++passedTests

  global.eq = deep-equal
  global <<< require 'prelude-ls'

  for file in fs.readdir-sync 'src/'
    require "./src/#file"

  process.on \exit ->
    time = ((Date.now! - startTime) / 1e3)toFixed 2
    message = "passed #passedTests tests in #time seconds"
    say if failedTests
    then tint "failed #failedTests and #message" red
    else tint message

  for filename in fs.readdir-sync 'test/'
    continue if filename in <[helpers.ls]>

    filename = "test/#filename"
    console.log "Running #{tint filename, bold}"
    c = code + "\n" + slurp(filename)
    try LiveScript.run c, {filename} catch
      ++failedTests
      return say e unless stk = e?stack
      msg = e.message or ''+ /^[^]+?(?=\n    at )/exec stk
      if m = /^(AssertionError:) "(.+)" (===) "(.+)"$/exec msg
        for i in [2 4] then m[i] = tint m[i]replace(/\\n/g \\n), bold 
        msg  = m.slice(1) * \\n
      [, row, col]? = //#{filename}:(\d+):(\d+)\)?$//m.exec stk
      if row and col
        say tint "#msg\n#{red}at #filename:#{row--}:#{col--}" red
        code = LiveScript.compile code, {+bare}
      else if /on line (\d+)\b/exec msg
        say tint msg, red
        row = that.1 - 1
        col = 0
      else return say stk
      {(row): line} = lines = code.split \\n
      lines[row] = line.slice(0 col) + tint line.slice(col), bold
      say lines.slice(row-8 >? 0, row+9) * \\n
